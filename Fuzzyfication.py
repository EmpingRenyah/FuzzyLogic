...
def fuzzify_service(service: int) -> list[double]:
    ...
    avg = 0.0
    good = 0.0
    if service < SERVICE_AVG[1]: # check for pure bad
        bad = 1.0
    elif service > SERVICE_AVG[4]: # check for pure good
        good = 1.0
    elif service > SERVICE_BAD[2] and service < SERVICE_GOOD[0]: # check for pure average
        avg = 1.0

    if service > SERVICE_BAD[1] and service < SERVICE_AVG[1]:
        bad = (SERVICE_BAD[2] - service) / (SERVICE_BAD[2] - SERVICE_BAD[1])
        avg = (service - SERVICE_AVG[0]) / (SERVICE_AVG[1] - SERVICE_AVG[0])
    
    if service > SERVICE_AVG[3] and service < SERVICE_GOOD[1]:
